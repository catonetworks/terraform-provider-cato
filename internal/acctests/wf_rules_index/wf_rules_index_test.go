...
// TestAccWfRulesIndex_WithRuleData lives in package wf_rules_index_with_rule_data so flaky
// CI (per-directory go test -timeout=5m) gives it its own budget alongside other WAN tests.
...
// wfRulesIndexTFs holds Terraform configs for section-only bulk move tests (indices 0–2).
// rule_data cases (former indices 3–4) live in package wf_rules_index_with_rule_data.
