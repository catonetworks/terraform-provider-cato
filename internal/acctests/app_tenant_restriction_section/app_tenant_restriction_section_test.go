...
package app_tenant_restriction_section
...
func TestAccAppTenantRestrictionSection(t *testing.T) {
	acc.SkipByEnv(t)
	mockSrv := accmock.NewMockServer(t, "TestAccAppTenantRestrictionSection")
	defer mockSrv.Close()
	mockSrv.Run()
	cfg := newSectionCfg(t)
	res := "cato_app_tenant_restriction_section.this"
...
func newSectionCfg(t *testing.T) sectionCfg {
	return sectionCfg{
		resName: acc.GetRandName("atr_section"),
		t:       t,
	}
}
...
var sectionTFs = []string{
	`resource "cato_app_tenant_restriction_section" "this" {
  at = {
