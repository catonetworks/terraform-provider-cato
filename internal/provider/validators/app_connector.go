...
func (v AppConTypeValidator) ValidateString(_ context.Context, req validator.StringRequest, resp *validator.StringResponse) {
	...
	if !connType.IsValid() {
		resp.Diagnostics.AddError("Field validation error", fmt.Sprintf("invalid connector type (%s: %s)\n - valid options: %+v",
			req.Path.String(), value, cato_models.AllZtnaAppConnectorType))
		return
	}
}
func (v AppConTypeValidator) Description(_ context.Context) string {
