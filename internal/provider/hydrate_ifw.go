...
func hydrateIfwRuleState(ctx context.Context, state InternetFirewallRule, currentRule *cato_go_sdk.Policy_Policy_InternetFirewall_Policy_Rules_Rule, resp *resource.ReadResponse, diags diag.Diagnostics) {
	...
	// Rule -> Schedule
	var scheduleInput Policy_Policy_InternetFirewall_Policy_Rules_Rule_Schedule
	// diags = ruleInput.Schedule.As(ctx, &scheduleInput, basetypes.ObjectAsOptions{})
	// resp.Diagnostics.Append(diags...)

	customScheduleSet := false
	...
	if currentRule.Schedule.GetCustomRecurringPolicySchedule() != nil {
		if currentRule.Schedule.GetCustomRecurringPolicySchedule().From != "" && currentRule.Schedule.GetCustomRecurringPolicySchedule().To != "" {
			...
			customRecurringInput.Days, diags = basetypes.NewListValueFrom(ctx, types.StringType, currentRule.Schedule.CustomRecurringPolicySchedule.Days)
			tflog.Info(ctx, "CUSTOM_RECURRING_SOMETHING", map[string]interface{}{
				"struct: ": customRecurringInput,
			})
			resp.Diagnostics.Append(diags...)
			if resp.Diagnostics.HasError() {
				return
			}
			objVal, diags := scheduleInput.CustomRecurring.ToObjectValue(ctx)
			resp.Diagnostics.Append(diags...)
			if resp.Diagnostics.HasError() {
				return
			}
			tflog.Info(ctx, "CUSTOM_RECURRING_SOMETHING_1", map[string]interface{}{
				"struct: ":       customRecurringInput,
				"type: ":         scheduleInput.CustomRecurring.AttributeTypes(ctx),
				"object_value: ": objVal.AttributeTypes(ctx),
			})

			objTypes := map[string]attr.Type{
				"from": types.StringType,
				"to":   types.StringType,
				// "days": customRecurringInput.Days.ElementType(ctx),
				"days": types.ListType{types.StringType},
			}

			scheduleInput.CustomRecurring, diags = types.ObjectValueFrom(ctx, objTypes, customRecurringInput)
			resp.Diagnostics.Append(diags...)
			if resp.Diagnostics.HasError() {
				return
			}

			tflog.Info(ctx, "CUSTOM_RECURRING_SOMETHING_2", map[string]interface{}{
				"struct: ": customRecurringInput,
			})
		}
	}
	tflog.Info(ctx, "CUSTOM_RECURRING_SOMETHING_3", map[string]interface{}{
		"latest_struct: ": scheduleInput,
	})
	...
	if customScheduleSet {
		...
		if scheduleInput.CustomTimeframe.IsNull() {
			...
			customeTimeFrameInput := Policy_Policy_InternetFirewall_Policy_Rules_Rule_Schedule_CustomTimeframe{}
			scheduleInput.CustomTimeframe, _ = basetypes.NewObjectValueFrom(ctx, objTypes, customeTimeFrameInput)
			resp.Diagnostics.Append(diags...)
			if resp.Diagnostics.HasError() {
				return
			}
		}
		tflog.Info(ctx, "CUSTOM_RECURRING_SOMETHING_4", map[string]interface{}{
			"latest_struct: ": scheduleInput,
		})
	...
	tflog.Info(ctx, "CUSTOM_RECURRING_SOMETHING_5", map[string]interface{}{
		"latest_struct: ": scheduleInput,
	})
	...
	if currentRule.Exceptions != nil {
		if len(currentRule.Exceptions) > 0 {
			...
			for _, item := range currentRule.Exceptions {
				...
				var itemExceptionsSourceInput Policy_Policy_InternetFirewall_Policy_Rules_Rule_Source
