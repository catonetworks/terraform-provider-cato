...
func (r *ifwRulesIndexResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	resp.Diagnostics.Append(diags...)
	var itemSourceIPRangeInput IfwRulesSectionItemIndex
	for _, item := range sectionDataFromPlanList {
		...
		sectionDataTmp := IfwRulesSectionDataIndex{
			SectionIndex: itemSourceIPRangeInput.SectionIndex.ValueInt64(),
			SectionName:  itemSourceIPRangeInput.SectionName.ValueString(),
		}
	...
	if len(plan.RuleData.Elements()) > 0 {
		ruleListFromPlan := make([]IfwRulesRuleItemIndex, 0, len(plan.RuleData.Elements()))
		diags = plan.RuleData.ElementsAs(ctx, ruleListFromPlan, false)
		resp.Diagnostics.Append(diags...)
		...
		for _, sectionNameItem := range listOfSectionNames {
			...
			for _, ruleItemFromPlan := range ruleListFromPlan {
				if ruleItemFromPlan.SectionName.String() == sectionNameItem {
					// section name -> rule index order -> rule name
					mapRuleIndexToSectionName[ruleItemFromPlan.IndexInSection.ValueInt64()] = ruleItemFromPlan.RuleName.ValueString()
				}
			}
			currentRuleId := ""
