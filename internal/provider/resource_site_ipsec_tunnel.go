...
func (r *siteIpsecTunnelResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if !plan.Primary.IsNull() {
		...
		// setting secondary
		if !plan.Secondary.IsNull() {
			input.Secondary = &cato_models.AddIpsecIkeV2TunnelsInput{}
			secondaryInput := &AddIpsecIkeV2TunnelsInput{}
			diags = plan.Secondary.As(ctx, &secondaryInput, basetypes.ObjectAsOptions{})
			resp.Diagnostics.Append(diags...)

			input.Secondary.DestinationType = (*cato_models.DestinationType)(secondaryInput.DestinationType.ValueStringPointer())
			input.Secondary.PopLocationID = secondaryInput.PopLocationID.ValueStringPointer()
			input.Secondary.PublicCatoIPID = secondaryInput.PublicCatoIPID.ValueStringPointer()
