...
page_title: "cato_bulk_lf_move_rule Resource - terraform-provider-cato"
subcategory: ""
description: |-
  Manages ordering of LAN firewall policy items.
  LAN Firewall Policy
  Section
  Network rule
  Firewall ruleSub-policy
  Section
  Network rule
  Firewall rule
---

# cato_bulk_lf_move_rule (Resource)
...
- `section_index` (Number) Position of the section in the policy or sub-policy. Starts with 1.
...
- `id` (String) Network rule ID or sub-policy scope rule ID
- `rule_type` (String) Rule type: POLICY_RULE, SUB_POLICY_SCOPE, SUB_RULE
